...
# layer_value=min(np.dot(g,atom) for atom in atoms)

# for atom in atoms:

#     value = np.dot(g, atom)

#     if np.isclose(value, layer_value):
...
# #                 plane_value = np.dot(g, atom)
# #                 plane = np.dot(g, spv[p])
# #                 if np.isclose(plane_value, plane):

# #                     basis1_x.append(atom[0])
# #                     basis1_y.append(atom[1])
# #                     basis1_z.append(atom[2])
...
for i in range(number_layers):
    ...
    for atom in layer_atoms:
        ...
        X.append(np.dot(atom, u))
        Y.append(np.dot(atom, v))
        Z.append(i*d)

    ax.scatter(X, Y, Z, s=70, label=f"Layer {i}")

ax.set_xlabel("Surface u")
ax.set_ylabel("Surface v")
